...
document.addEventListener('DOMContentLoaded', function () {
    document.querySelectorAll('.video-wrapper, .video-wrapper-short').forEach(wrapper => {
        const id = wrapper.getAttribute('data-id');

        // Set initial thumbnail + play button
        wrapper.innerHTML = `
        ...
        wrapper.addEventListener('click', function () {
            // Stop all other videos (both horizontal & vertical)
            document.querySelectorAll('.video-wrapper iframe, .video-wrapper-short iframe').forEach(frame => {
                const parent = frame.parentNode;
                const otherId = parent.getAttribute('data-id');
                parent.innerHTML = `
                    <img src="https://img.youtube.com/vi/${otherId}/hqdefault.jpg" alt="Video Thumbnail" 
                        style="position:absolute;top:0;left:0;width:100%;height:100%;object-fit:cover;">
                    <div class="play-button"></div>
                `;
            });
